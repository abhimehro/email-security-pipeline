...
class CodeSceneRefactoringAgent:
    ...
    def _get_confidence_threshold(self, skill: str) -> float:
        ...
        return self.SKILL_CONFIDENCE_MAP.get(skill, self.SKILL_CONFIDENCE_MAP["default"])
...
- Consider: Is this a safe, non-breaking change?"""
            ...
            if applicability != "applicable":
                self.log("INFO", f"Not applicable for skill '{self.skill}': {result.get('reason')}")
                return "not_applicable", 0.0
            
            confidence = result.get("fix_confidence", 0.0)
            can_fix = result.get("can_auto_fix", False)
            
            self.log("INFO", f"Issue classification: can_auto_fix={can_fix}, confidence={confidence:.2f}")
...
Return the fixed code:"""
            ...
            if fixed_code.startswith("```"):
                ...
                if fixed_code.startswith(("python", "javascript", "java", "go", "typescript", "rust")):
...
Be concise and actionable."""
    ...
    def create_pr_comment(self) -> str:
        ...
        threshold_info = f"Auto-fix confidence threshold: ≥{int(self.auto_fix_threshold*100)}%"
    ...
    def run(self) -> bool:
        ...
        for file_info in files_to_improve:
            ...
            if code_health < 8.0:  # Focus on files with health < 8.0
                ...
                if applicability != "applicable":
                    self.log("INFO", f"Skipping {file_path} - not applicable for skill '{self.skill}'")
                    continue
                
                # Decide based on confidence and skill threshold
                if confidence >= self.auto_fix_threshold:
                    self.log("INFO", f"High confidence fix for {file_path} ({confidence:.2f})")
                    fixed_code = self.generate_auto_fix(file_path, code_health)
                    if fixed_code:
                        self.apply_auto_fix(file_path, fixed_code)
                else:
                    self.log("INFO", f"Generating suggestion for {file_path} (confidence: {confidence:.2f})")
