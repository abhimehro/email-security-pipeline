...
The current workflow files in this repo use the `email-security-pipeline` slot from the staggered schedule plan below.
...
- `email-security-pipeline` has no live-state exception like `ctrld-sync`, but protected paths such as `src/auth/**` and credential-adjacent logic should still stay manual-review only.
