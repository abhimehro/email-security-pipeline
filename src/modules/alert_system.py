...
def _safe_requests_post(url: str, **kwargs) -> requests.Response:
    ...
    # is_safe_webhook_url is already imported globally
    is_safe, err = is_safe_webhook_url(url)
    if not is_safe:
        raise requests.exceptions.ConnectionError(f"SSRF attempt blocked: {err}")
