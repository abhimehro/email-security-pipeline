...
class SpamAnalyzer:
    ...
    def _analyze_single_url(url: str) -> Tuple[float, int]:
        """
        Analyze a single URL for suspicion with caching.
        ...
        try:
            ...
            if SpamAnalyzer.COMBINED_URL_PATTERN.search(domain):
                current_url_score += 0.5
                append_count += 1

            # Check for URL shorteners
            if SpamAnalyzer.SHORTENER_PATTERN.search(domain):
                current_url_score += 0.5
                append_count += 1
