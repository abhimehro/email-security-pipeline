...
class SpamAnalyzer:
    ...
    HIDDEN_TEXT_PATTERN = re.compile(
        r"font-size:\s*[0-2]px|color:\s*#fff.*background.*#fff", re.IGNORECASE
    )
    ...
    COMBINED_URL_PATTERN = re.compile(
        "|".join(p.pattern for p in SUSPICIOUS_URL_PATTERNS), re.IGNORECASE
    )
    ...
    def _analyze_subject(self, subject: str) -> Tuple[float, List[str]]:
        ...
        if self.COMBINED_SPAM_PATTERN.search(subject_lower):
            ...
            for pattern_re, pattern_str in zip(
                self.SPAM_KEYWORD_REGEXES, self.SPAM_KEYWORDS
            ):
    ...
    def _check_urls(self, contents: Union[str, List[str]]) -> Tuple[float, List[str]]:
        """Check for suspicious URLs"""
        score = 0.0
        suspicious = []
        ...
        for content in contents:
            if content:
                urls.extend(self.URL_EXTRACTION_PATTERN.findall(content))

        # Optimization: Deduplicate for analysis but preserve counts for scoring
