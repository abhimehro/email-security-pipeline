...
def signal_handler(signum, frame):
    ...
    print(f"\n{Colors.YELLOW}Received shutdown signal, stopping gracefully...{Colors.RESET}")
...
def main():
    ...
    if not Path(config_file).exists():
        ...
        print(f"You can run: {Colors.BOLD}cp .env.example .env{Colors.RESET}")
        sys.exit(1)
    ...
    try:
        with open(config_file, 'r') as f:
            ...
            if 'your-email@gmail.com' in content or 'your-app-password-here' in content or 'your-email@outlook.com' in content or 'your-bridge-password-here' in content:
                print(f"{Colors.YELLOW}Warning: .env file appears to contain example values.{Colors.RESET}")
                print("Please update .env with your actual credentials before running.")
                sys.exit(1)
    except Exception as e:
        print(f"{Colors.YELLOW}Warning: Could not validate .env file: {e}{Colors.RESET}")
