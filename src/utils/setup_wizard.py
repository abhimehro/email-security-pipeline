...
def _print_provider_help(choice: str) -> None:
    ...
    if choice == "1":  # Gmail
        print("\n" + Colors.colorize("Note: Use an App Password, not your login password.\nGenerate one at: Google Account -> Security -> 2-Step Verification -> App passwords", Colors.GREY))
    elif choice == "2":  # Proton
        print("\n" + Colors.colorize("Note: Use the password generated by Proton Mail Bridge.\nFind it in: Proton Mail Bridge app -> Mailbox details", Colors.GREY))
    elif choice == "3":  # Outlook
        print("\n" + Colors.colorize("Note: Personal Outlook accounts are not supported.\nBusiness accounts may require an App Password if MFA is enabled.", Colors.GREY))
...
def _get_credentials(choice: str, provider_name: str) -> tuple[str, str]:
    ...
    print(
        "\n" + Colors.colorize(f"Step 2 of 2: Configure {provider_name} Credentials", Colors.CYAN)
    )
