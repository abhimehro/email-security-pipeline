...
def run_setup_wizard(config_file: str = ".env", template_file: str = ".env.example") -> bool:
    ...
    # Provider Selection
    print(f"{Colors.CYAN}Step 1: Choose your email provider{Colors.RESET}")
    ...
    print(f"\n{Colors.CYAN}Step 2: Configure {selected_name} Credentials{Colors.RESET}")

    try:
        email = input(f"Enter your {selected_name} email address: ").strip()
        while not email:
        ...
        password = getpass.getpass(f"Enter your {selected_name} app password: ").strip()
        while not password:
    ...
    for provider in all_providers:
        if provider == selected_key:
            # Enable selected
            # Regex handles both true/false in template
    ...
    content = re.sub(f"{selected_key}_EMAIL=.*", f"{selected_key}_EMAIL={email}", content)

    # Set password for selected provider safely
    def replace_password(match):
        return f"{selected_key}_APP_PASSWORD={password}"

    content = re.sub(f"{selected_key}_APP_PASSWORD=.*", replace_password, content)
