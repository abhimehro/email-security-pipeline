...
# Connection utilities for verification
try:
...
def _test_connection(email: str, app_password: str, provider_choice: str) -> bool:
    ...
    """
    # Only test if IMAPConnection is available (imports succeeded)
    if 'IMAPConnection' not in globals():
        return True
    ...
    if provider_choice == '1': # Gmail
        ...
        imap_port = 1143
        provider = "proton"
        use_ssl = True # Per default config
    ...
    try:
        # Use Spinner if available, otherwise just text
...
def _get_credentials(choice: str, provider_name: str) -> tuple[str, str]:
    ...
    try:
        while True:
            ...
            # If verification failed, ask to retry
