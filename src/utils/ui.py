...
def _get_terminal_columns(columns: int) -> int:
    """Helper to resolve terminal columns if not explicitly provided."""
    if columns <= 0:
        return shutil.get_terminal_size((80, 20)).columns - 1
    return columns


def _truncate_plain_text(text: str, columns: int) -> str:
    """Helper to truncate plain text without ANSI escape sequences."""
    if len(text) <= columns:
        return text
    if columns <= 0:
        return ""
...
def _truncate_ansi_text(
    text: str, parts: list, columns: int, total_visual_length: int
) -> str:
    """Helper to truncate text containing ANSI escape sequences."""
    ...
    for i, part in enumerate(parts):
        part_len = len(part)
        if visual_length + part_len > columns:
            allowed = max(0, columns - visual_length)
            if allowed > 0:
                result.append(part[:allowed])
            break

        result.append(part)
        visual_length += part_len

        if i < len(escapes):
            result.append(escapes[i])

    suffix = "\033[0m" if visual_length < total_visual_length else ""
...
def _truncate_for_terminal(text: str, columns: int = 0) -> str:
    ...
    columns = _get_terminal_columns(columns)

    # Fast path for plain text without ANSI escape sequences
    if "\x1b" not in text:
        return _truncate_plain_text(text, columns)
    ...
    return _truncate_ansi_text(text, parts, columns, total_visual_length)
...
class CountdownTimer:
    ...
    def _format_time_str(self, remaining: int) -> str:
        """Format time as MM:SS if initial duration >= 60s, else formatted seconds."""
        if self.duration >= 60:
            return f"{remaining // 60:02d}:{remaining % 60:02d}"
        width = len(str(self.duration))
        return f"{remaining:{width}d}s"

    def _render_timer_frame(self, remaining: int, columns: int) -> None:
        """Render a single frame of the countdown progress bar."""
    ...
    def _handle_interrupt(self) -> None:
        """Clean up and print cancellation message on interrupt."""
    ...
    def start(self):
        ...
        try:
            ...
            while remaining > 0 and not self._stop_event.is_set():
                self._render_timer_frame(remaining, columns)
