...
## 2023-10-24 - Parallelize Attachment Media Analysis
**Learning:** Sequential processing of independent I/O or CPU-bound tasks per item (like attachments) is a significant bottleneck.
**Action:** Use `ThreadPoolExecutor.map()` to process independent items concurrently while preserving deterministic return order and naturally propagating exceptions. To maintain sequential short-circuiting logic on a cumulative metric (like threat_score), use a shared atomic state dictionary (`{"stop": False}`) to signal early exit across threads without blocking.
