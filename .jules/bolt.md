...
**Learning:** Instantiating a new `ThreadPoolExecutor` inside a hot loop (like a
per-email analysis step) causes massive OS-level thread allocation overhead. A
synthetic benchmark proved that allocating it per-call took ~1.98s per 1000
iterations compared to ~0.39s when re-using a pre-allocated pool. **Action:**
