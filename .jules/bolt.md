...
## 2025-11-15 - Numpy vs OpenCV for Image Statistics
**Learning:** `np.std(frame.astype(float))` involves a massive memory allocation and copy (~48MB for 1080p), whereas `cv2.meanStdDev(frame)` operates directly on `uint8` data and is ~28x faster. Also, `cv2.countNonZero` is ~12x faster than `np.sum` for binary masks.
**Action:** Use OpenCV optimized functions (`meanStdDev`, `countNonZero`) instead of numpy equivalents when processing images, especially in loops.
