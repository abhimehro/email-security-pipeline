...
def print_status(protocol, host, port, use_ssl, success, message=None):
    ...
    if not success and message:
        ...
        # Optional debug info for success
        pass
    ...
    try:
        if use_ssl:
            ...
            with imaplib.IMAP4_SSL(host, port, ssl_context=ctx) as imap:
                ...
                typ, data = imap.noop()
                print_status("IMAP", host, port, use_ssl, True, f"{typ} {data}")
        else:
...
def main():
    ...
    if os.getenv("PROTON_ENABLED", "false").lower() == "true":
        ...
        # Determine if we should use SSL or STARTTLS for Proton
        # Usually Bridge 1143 is STARTTLS/None and 143 might be STARTTLS too depending on config.
        # The original script had 1143 as False (STARTTLS) and 1025 as False (STARTTLS).
        # We will respect the env var or default to STARTTLS (False) for Proton as per original.
