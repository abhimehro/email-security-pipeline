...
class TestEmailIngestionSecurity(unittest.TestCase):
    ...
    def test_select_folder_log_injection(self):
        """Test that folder selection logs are sanitized"""
        # Malicious folder name with newline (Log Injection)
        ...
        # Check that logger was called with sanitized version
        expected_folder_log = sanitize_for_logging(malicious_folder)

        # Verify debug call
        found_sanitized = False
        for call in self.client.logger.debug.call_args_list:
            args, _ = call
            if expected_folder_log in args[0] and "\n" not in args[0]:
    ...
    def test_parse_email_filename_log_injection(self):
        ...
        expected_filename = sanitize_for_logging(malicious_filename)
