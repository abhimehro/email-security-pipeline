...
class TestMediaOptimization(unittest.TestCase):
    ...
    def test_extract_frames_sequential_read_optimization(self, mock_capture):
        ...
        frames = self.analyzer._extract_frames_from_video(
            "dummy.mp4", max_frames=max_frames
        )
        ...
        set_calls = [
            call
            for call in cap_instance.set.call_args_list
            if call[0][0] == cv2.CAP_PROP_POS_FRAMES
        ]

        self.assertEqual(
            len(set_calls), 0, "Should not call set() for sequential reading (step=1)"
        )
    ...
    def test_extract_frames_seek_for_large_step(self, mock_capture):
        ...
        self.assertGreater(
            len(set_calls), 0, "Should call set() for non-sequential reading (step>1)"
        )
