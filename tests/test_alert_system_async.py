...
class TestAsyncAlertDispatch(unittest.TestCase):
    ...
    def test_alert_ordering_preserved(self, mock_safe, mock_post):
        ...
        def capture_call(*args, **kwargs):
            ...
            m = MagicMock()
            m.__aenter__ = AsyncMock(return_value=MagicMock(status=200))
...
class TestProcessSingleAlertWithRetry(unittest.IsolatedAsyncioTestCase):
    ...
    def _make_config(self, webhook=False):
        ...
        config.slack_webhook = (
            "https://hooks.slack.com/services/T0000/B0000/XXXX" if webhook else None
        )
        return config
    ...
    async def test_retry_on_exception(self, mock_sleep):
        ...
        system._dispatch_alert_async = AsyncMock(
        ...
        self.assertTrue(
            any("failed" in str(c[1][0]) for c in system.logger.error.mock_calls)
        )
        self.assertFalse(
            any("permanently" in str(c[1][0]) for c in system.logger.error.mock_calls)
        )
    ...
    async def test_retry_on_timeout(self, mock_sleep):
        ...
        system._dispatch_alert_async = AsyncMock(
            side_effect=[asyncio.TimeoutError("timeout"), None]
        )
        ...
        mock_sleep.assert_called_with(1)  # 2**0
        self.assertTrue(
            any("timed out" in str(c[1][0]) for c in system.logger.error.mock_calls)
        )
        self.assertFalse(
            any("permanently" in str(c[1][0]) for c in system.logger.error.mock_calls)
        )
    ...
    async def test_permanent_failure(self, mock_sleep):
        ...
        system._dispatch_alert_async = AsyncMock(
            side_effect=[Exception("fail 1"), Exception("fail 2"), Exception("fail 3")]
        )
        ...
        self.assertTrue(
            any(
                "permanently failed" in str(c[1][0])
                for c in system.logger.error.mock_calls
            )
        )
